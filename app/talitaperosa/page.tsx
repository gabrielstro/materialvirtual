...
const onClickCTA = (goToCheckout?: boolean) => {
  sendGTMEvent({ event: 'clickCTA', value: 'toCTAsection' })
  let element = document.getElementById('cta');
  if(element && !goToCheckout) element.scrollIntoView({ behavior: "smooth", block: "start"});
      ...
      <section className="bg-center bg-cover bg-[url('/lp/talitaperosa/bg06desk.webp')] min-w-full md:h-[700px] h-[1000px] flex" id="cta">
        <div className="md:w-[984px] xl:w-[1170px] p-5 w-full h-full m-auto flex md:flex-row flex-col items-center justify-center">
          <div className="h-[499px] md:w-[421px] border border-[#691817] rounded-lg bg-[#F8EFE8] flex flex-col p-6 text-center text-2xl gap-5">
            ...
            <button onClick={() => onClickCTA(true)} className="bg-gradient-to-r from-[#620D19] to-[#B8162D] md:w-[328px] w-full text-white text-xl rounded-sm md:px-8 py-4 my-4 mx-auto">Quero minha vaga agora</button>
